...
function render (body, props) {
  ...
  if (process.browser) {
    // Updating the <head> resets the title, so we need to do this
    // on the next tick, once the new <head> has been inserted in the DOM
    window.setTimeout(function () { updateTitleFromHead(head) }, 0)
  }
